  ...
  return (
    <div className="min-h-screen bg-[#f8f7ff]">
      ...
      <main className="p-8 max-w-[1200px] mx-auto">
        <Link
          to={`/share/${token}`}
          className="flex items-center gap-2 text-[#334155] hover:text-[#0d9488] transition-all hover:-translate-x-1 font-bold mb-6 text-sm font-mono"
        >
        ...
        <div className="flex items-start justify-between mb-6 gap-4">
          <div>
            <h2 className="text-2xl font-bold text-zinc-900">{callLog?.contact?.name || report?.contactName || 'Unknown'}</h2>
            <p className="text-zinc-500 text-sm mt-0.5 font-mono">{callLog?.contact?.phone} · {new Date(callLog?.createdAt || report?.createdAt).toLocaleString()}</p>
          </div>
          {callLog?.durationMs && (
            <span className="text-xs font-mono bg-zinc-100 text-zinc-600 px-3 py-1.5 rounded-full shrink-0">
        ...
        {report && (
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-white border border-zinc-200 p-5 rounded-xl shadow-sm">
              <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider mb-2">Outcome</p>
              <span className={`text-xs font-semibold px-2 py-1 rounded-full ${OUTCOME_BADGE[report.outcome] || 'bg-zinc-100 text-zinc-600'}`}>
                {(report.outcome || 'Unknown').replace('_', ' ')}
              </span>
            </div>
            <div className="bg-white border border-zinc-200 p-5 rounded-xl shadow-sm">
              <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider mb-2">QA Score</p>
              <p className="text-3xl font-bold text-[#0d9488]">{report.score ?? '—'}<span className="text-zinc-400 text-base font-semibold">/100</span></p>
            </div>
            <div className="bg-white border border-zinc-200 p-5 rounded-xl shadow-sm">
              <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider mb-2">Sentiment</p>
              <span className={`text-xs font-semibold px-2 py-1 rounded-full ${SENTIMENT_BADGE[report.sentiment] || 'bg-zinc-100 text-zinc-700'}`}>
                {report.sentiment ? report.sentiment.charAt(0).toUpperCase() + report.sentiment.slice(1) : '—'}
              </span>
            </div>
            <div className="bg-white border border-zinc-200 p-5 rounded-xl shadow-sm">
              <p className="text-xs text-zinc-500 font-mono uppercase tracking-wider mb-2">Completion</p>
              <p className="text-3xl font-bold text-zinc-900">{completionPercent != null ? `${completionPercent}%` : '—'}</p>
            </div>
          </section>
        )}
        ...
        {report?.reportSummary && (
          <div className="bg-white border border-zinc-200 rounded-xl shadow-sm p-6 mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-lg font-semibold text-zinc-900">AI Summary</h3>
            </div>
            <p className="text-zinc-700 leading-relaxed">{report.reportSummary}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
          ...
          {callLog?.hasRecording && (
            <div className="bg-white border border-zinc-200 rounded-xl shadow-sm p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-zinc-900">Recording</h3>
              </div>
              <audio
                controls
                className="w-full"
          ...
          {/* Transcript */}
          <div className={`bg-white border border-zinc-200 rounded-xl shadow-sm p-6 ${callLog?.hasRecording ? '' : 'xl:col-span-2'}`}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                {turns.map((t, i) => (
                  <div key={i} className={`flex gap-3 ${t.isAI ? 'flex-row' : 'flex-row-reverse'}`}>
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-[10px] font-bold ${t.isAI ? 'bg-[#0f766e] text-white' : 'bg-zinc-200 text-zinc-600'}`}>
                      {t.isAI ? 'AI' : 'U'}
                    </div>
                    <div className={`px-4 py-2.5 rounded-2xl max-w-[80%] text-sm leading-relaxed ${t.isAI ? 'bg-[#e6fffa] text-zinc-800 rounded-tl-sm' : 'bg-zinc-100 text-zinc-700 rounded-tr-sm'}`}>
                      {t.text}
                    </div>
                  </div>
                ))}
        ...
        {questionResults.length > 0 && (
          <FullscreenTable className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-6 border-b border-zinc-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <h3 className="text-lg font-semibold text-zinc-900">Evaluation Breakdown</h3>
              ...
              <div className="flex bg-zinc-100 p-1 rounded-lg">
                {[['all','All'],['full','Full Score'],['partial','Partial'],['failed','Failed']].map(([key, label]) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors font-mono ${filterScore === key ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-600 hover:text-zinc-900'}`}
                  >
                    {label}
                  </button>
                ))}
              ...
              <table className="w-full text-left">
                <thead className="bg-zinc-50 border-b border-zinc-100">
                  <tr>
                    {['Question', 'Answer', 'Scoring Rule', 'Points'].map(h => (
                      <th key={h} className="px-6 py-3 text-xs text-zinc-500 uppercase tracking-wider font-mono">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-zinc-50">
                  {filteredQuestions.map(qr => {
                    const max = qr.weight || 0;
                    const awarded = qr.questionScore || 0;
                    const color = max === 0 ? 'text-zinc-600' : awarded >= max ? 'text-emerald-600' : awarded === 0 ? 'text-[#ba1a1a]' : 'text-amber-600';
                    ...
                    return (
                      <tr key={qr.questionId} className="hover:bg-zinc-50/70 transition-colors">
                        <td className="px-6 py-4 max-w-xs">
                          <p className="text-sm font-medium text-zinc-800 leading-snug">{qr.questionText || mainRow.questionText || qr.questionId}</p>
                        </td>
                        <td className="px-6 py-4">
                          {qr.answerExtracted ? (
                            <div>
                              <p className="text-sm text-zinc-800">{qr.answerExtracted}</p>
                              {conf && (
                                <div className="flex items-center gap-2 mt-1">
                                  ...
                                  <span className="text-[10px] text-zinc-400 uppercase font-mono">{confStr}</span>
                                </div>
                              )}
                            </div>
                          ) : (
                            <span className="text-xs text-zinc-400 italic">No answer</span>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <span className="text-xs text-zinc-500 font-mono">{mainRow.rule || '—'}</span>
                          {mainRow.reason && <p className="text-[10px] text-zinc-400 mt-0.5">{mainRow.reason}</p>}
                        </td>
