    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-[#e7e5e0]">
      <nav className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between" aria-label="Primary">
        ...
        <ul className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map(l => (
            <li key={l.href}>
              <a href={l.href} className="text-sm font-medium text-[#4b5148] hover:text-[#14261f] transition-colors">
                {l.label}
              </a>
            </li>
          ))}
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link to="/login" className="text-sm font-medium text-[#4b5148] hover:text-[#14261f] transition-colors px-3 py-2 rounded-lg cursor-pointer">
            Sign in
          </Link>
          <Link
            to="/login"
        ...
        <button
          ...
          className="md:hidden w-10 h-10 flex items-center justify-center text-[#14261f] rounded-lg hover:bg-black/5 cursor-pointer"
        >
      ...
      {open && (
        <div id="mobile-menu" className="md:hidden border-t border-[#e7e5e0] px-6 py-4 flex flex-col gap-1">
          {NAV_LINKS.map(l => (
            <a
              key={l.href}
              href={l.href}
              onClick={() => setOpen(false)}
              className="text-sm font-medium text-[#4b5148] hover:text-[#14261f] py-2.5"
            >
              {l.label}
            </a>
          ))}
          <button
            ...
            className="text-left text-sm font-medium text-[#4b5148] hover:text-[#14261f] py-2.5 flex items-center gap-1.5"
          >
            <Compass size={16} />
            Take a tour
          </button>
